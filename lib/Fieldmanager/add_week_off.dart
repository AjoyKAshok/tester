...
if(dropdownmerch!=null&&dropdownmonth!=null&&dropdowndays!=null&&dropdownyear!=null)
  {
    Addweekoff.empid =
    'RMS${dropdownmerch.replaceAll(new RegExp(r'[^0-9]'), '')}';
  ...
  {
    Flushbar(
        message: dropdownmerch == null
            ? "Merchandiser cannot be empty"
            : dropdownmonth == null
            ? "Month cannot be empty"
            :dropdowndays == null? "Day cannot be empty":"Year cannot be empty",
...
class _SelectMerchState extends State<SelectMerch> {
  static List DropDownItems = currentuser.roleid == 5
      ? merchnamelist.firstname.map((String val) {
          return new DropdownMenuItem<String>(
            value: val,
            child: new Text(val),
          );
        }).toList()
      : MerchUnderCDE.firstname.map((String val) {
          return new DropdownMenuItem<String>(
            value: val,
            child: new Text(val),
          );
...
class _SelectYearState extends State<SelectYear> {
  static List DropDownItems =
      ["2022", "2023", "2024", "2025"].map((String val) {
    return new DropdownMenuItem<String>(
      value: val,
      child: new Text(val),
    );
