      ...
      return Column(
        ...
        children: <Widget>[
            ...
            child: TextFormField(
             ...
             onChanged: (String newValue){
               if (items != null) {
                 if (strict &&
                     newValue.isNotEmpty &&
                     ! items.toString().trim().toUpperCase().contains(newValue.toString().trim().toUpperCase()))
                 {
                   showDrop= false;
                   ///testing
                   // categoryController.clear();
                 ...
                 {
                   int res = items.where((e) =>  e.toString().trim().toUpperCase().contains(newValue.toString().trim().toUpperCase()) ).length;
                   searchingCount = res;
                   print("COUNT...$res.......${newValue.toString().toUpperCase()}");
                   showDrop= true;
          ...
          Container(
            ...
            height:searchingCount>=3? 3 *
                48.0:searchingCount==2?2*48.0:searchingCount==1?1*48.0:0,
            // height:state._items.length>=3? 3 *
            //     48.0:state._items.length==2?2*48.0:state._items.length==1?1*48.0:0, //limit to default 3 items in dropdownlist view and then remaining scrolls
  ...
  List<ListTile> _getChildren(List<String> items) {
    ...
    for (var item in items) {
      if (_searchText.isNotEmpty) {
        ...
        if (item.toString().trim().toUpperCase().contains(_searchText.toString().trim().toUpperCase()))
          {
