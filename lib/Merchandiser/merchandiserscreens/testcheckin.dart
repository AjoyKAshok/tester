...
import 'package:merchandising/Merchandiser/merchandiserscreens/testactivitiespage.dart';
...
class TestCheckIn extends StatefulWidget {
  const TestCheckIn({ Key key }) : super(key: key);

  @override
  State<TestCheckIn> createState() => _TestCheckInState();
}

class _TestCheckInState extends State<TestCheckIn> {
    ...
    return GestureDetector(
      onTap: () async {
        ...
        if (chekinoutlet.currentdistance > 400) {
          ...
          //     content: const Text(
          //         "The Check Out Feature will be available within a 300 Meter radius of the store...\n \n Please Check Out only after completing all activities in the store..."),
          //     actions: <Widget>[
          //       TextButton(
          //         onPressed: () {
        ...
        //       MaterialPageRoute(builder: (BuildContext dialogContext) {
        //     return NewCustomerActivities();
        //   }), (Route<dynamic> route) => false);
        // }
        Navigator.pushAndRemoveUntil(context,
              MaterialPageRoute(builder: (BuildContext dialogContext) {
            return TestActivitiesPage();
          }), (Route<dynamic> route) => false);
        }
      },
      child: Container(
...
class NewForceCheckin extends StatefulWidget {
  @override
  _NewForceCheckinState createState() => _NewForceCheckinState();
}

class _NewForceCheckinState extends State<NewForceCheckin> {
  bool isApiCallProcess = false;
                                        ...
                                        child: GestureDetector(
                                          onTap: () async {
                                            ...
                                            if (gpsnotworking == true) {
                                              ...
                                              //     content: const Text(
                                              //         "Check Out Feature will be Available only with in 300 Meter Radius of the Store...\n \n Please Check Out only after completing all activities in the store..."),
                                              //     actions: <Widget>[
                                              //       TextButton(
                                              //         onPressed: () async {
                                              //           // Navigator.of(ctx).pop();
                                              ...
                                              if (geolocation == true) {
                                                ...
                                                print(
                                                    'Check In Location from Checkin Button: ${checkinoutdata.checkinlocation}');
                                                // Navigator.pushAndRemoveUntil(
                                                //     context,
                                                //     MaterialPageRoute(builder:
                                                //         (BuildContext context) {
